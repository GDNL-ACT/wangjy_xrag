...
# ## yahoo_qa
# data = datasets.load_dataset('yahoo_answers_qa', revision='refs/convert/parquet')
# print(len(data['train']))
# print(data['train'][0])
# for idx,sample in enumerate(data['train']):
#     question = sample['question'] + '\n'
...
# ## freebase_qa
# data = datasets.load_dataset('freebase_qa', revision='refs/convert/parquet')
# print(len(data['train']))
# print(data['train'][0])
# for idx,sample in enumerate(data['train']):
...
## samsum
dataset = datasets.load_dataset("knkarthick/samsum")
...
## pwc
dataset = datasets.load_dataset("sggetao/PwC", revision='refs/convert/parquet', data_files="default/train/*.parquet")["train"]
print(len(dataset))
...
# ## triviaqa
# tqa = [json.loads(x) for x in open("data/eval/triviaqa/triviaqa_train.jsonl").readlines()]
# print(len(tqa))
...
# 保存total_data到指定路径
output_file = "data/instruction_tuning/processed/context_aware_instrution_tuning_data.jsonl"
with open(output_file, 'w', encoding='utf-8') as f:
    for item in total_data:
        f.write(json.dumps(item, ensure_ascii=False) + '\n')

print(f"总共保存了 {len(total_data)} 条数据到 {output_file}")
