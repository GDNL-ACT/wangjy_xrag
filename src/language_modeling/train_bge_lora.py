...
def main():
    ...
    if args.retriever_name_or_path is not None:
        retriever = BGE(args.retriever_name_or_path, device = accelerator.device, enable_lora=True)
        ...
        accelerator.print("Retriever LoRA enabled:")
        retriever.get_trainable_parameters()
    ...
    if args.update_projector_only:
        ...
        if  retriever is not None:
            params_to_optimize.extend([p for p in retriever.model.parameters() if p.requires_grad])
        ...
        # 如果启用了retriever LoRA，添加retriever参数组
        if retriever is not None:
            optimizer_grouped_parameters.extend([
                {
                    "params": [p for n, p in retriever.model.named_parameters() if p.requires_grad and not any(nd in n for nd in no_decay)],
                    "weight_decay": args.weight_decay,
                },
                {
                    "params": [p for n, p in retriever.model.named_parameters() if p.requires_grad and any(nd in n for nd in no_decay)],
                    "weight_decay": 0.0,
                },
            ])
    ...
    if dev_dataset is not None:
        model, retriever.model, optimizer, train_dataloader, lr_scheduler, dev_dataloader = accelerator.prepare(
        model, retriever.model, optimizer, train_dataloader, lr_scheduler, dev_dataloader)

    else:
        model, retriever.model, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        model, retriever.model, optimizer, train_dataloader, lr_scheduler)
            ...
            if accelerator.sync_gradients:
                ...
                if isinstance(checkpointing_steps, int):
                    if completed_steps % checkpointing_steps == 0:
                        ...
                        save_with_accelerate(accelerator, model, tokenizer, output_dir,save_projector_only=args.update_projector_only, retriever=retriever)
    ...
    save_with_accelerate(accelerator, model, tokenizer, output_dir,save_projector_only=False, retriever=retriever)
